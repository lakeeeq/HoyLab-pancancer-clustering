...
# selecting the pathway of interest 
pathway <- rownames(input)
selected_pathway <- c( pathway[ grep("Alanine..aspartate", pathway) ], 
                       pathway[ grep("TCA", pathway) ], 
                       pathway[ grep("Glycolysis", pathway) ],
                       #  pathway[ grep("Pentose", pathway) ],
                       pathway[ grep("Pyruvate", pathway) ] ,
                       pathway[ grep("Oxidative", pathway) ] )

input <- input[ selected_pathway, ]


# subset to cluster 3 and cluster 4 cell lines
...
# select the top 20 unique drugs
tT_no_same_drug <- tT
...
# write.xlsx( data.frame(input), "figureS3b_down_scaled_fitness.xlsx", rowNames=TRUE)
