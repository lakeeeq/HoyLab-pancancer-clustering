...
input  <- data.frame( read.xlsx("Figure4e_left/figure4e_left_input.xlsx")) 
input[1:10, 1:10]
...
# cluster 3 and cluster 4 cell lines

cluster_3_cellline <- c( "NCIH226", "HT144" , "PC3",    "A375"    ,  "WM2664" , 
                         "DU145"  , "Detroit562" ,       "BT474" , "A549"     )
...
tT_no_same_drug <- tT 
tT_no_same_drug$drug <- unlist(lapply(strsplit(rownames(tT_no_same_drug ), ".split."), `[` ,2))
...
Heatmap(matrix =  input  ,   top_annotation = ha, 
...
# write.xlsx( data.frame(input  ), "figure4e_left_scaled_AUC.xlsx", rowNames=TRUE)
