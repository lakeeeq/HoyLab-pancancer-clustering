...
##########
 ...
 # shows the glycolysis group 
 output <-  as.matrix( t(data_for_clustering ) ) [  c( 24, 16, 17,  25,  26 : 32),   order_cellline ] 
 
 Heatmap(matrix =  output    , 
         top_annotation = ha[order_cellline], 
...
# write.xlsx( data.frame( output)  , "figure2a_scaled_expression.xlsx", rowNames=TRUE)
...
##########
 ...
 # shows the pentose phosphate pathway
 output <-   as.matrix( t(data_for_clustering ) ) [ 16 : 23,   order_cellline ]  
Heatmap(matrix = output   , 
              top_annotation = ha[order_cellline], 
              row_title_gp = gpar(fontsize = 6),
              cluster_columns = F,   cluster_rows = T, 
...
# write.xlsx( data.frame( output)  , "figure2b_scaled_expression.xlsx", rowNames=TRUE)
...
# shows the TCA cycle 
output <-  as.matrix( t(data_for_clustering ) ) [  1: 15,   order_cellline ]
 Heatmap(matrix =  output  , 
         top_annotation = ha[order_cellline], 
         row_title_gp = gpar(fontsize = 6),
         cluster_columns = F,   cluster_rows = T, 
...
# write.xlsx( data.frame( output)  , "figure2c_scaled_expression.xlsx", rowNames=TRUE)
 ...
 # shows the proline metabolism 
 output <-   as.matrix( t(data_for_clustering ) ) [  51 : 68,   order_cellline ] 
 Heatmap(matrix =   output , 
         top_annotation = ha[order_cellline], 
         row_title_gp = gpar(fontsize = 6),
         cluster_columns = F,   cluster_rows = T, 
 ...
 # write.xlsx( data.frame( output)  , "figure2d_scaled_expression.xlsx", rowNames=TRUE)
...
##########
 ...
 # shows the serine metabolism 
 output <-   as.matrix( t(data_for_clustering ) ) [  33 : 50 ,   order_cellline ] 
 ...
 # write.xlsx( data.frame( output)  , "figure2e_scaled_expression.xlsx", rowNames=TRUE)
 ...
 # shows the glutamine metabolism 
 output <-   as.matrix( t(data_for_clustering ) ) [ 87 : ncol(data_for_clustering ),   order_cellline ] 
...
# write.xlsx( data.frame( output)  , "figure2f_scaled_expression.xlsx", rowNames=TRUE)
 ...
 # shows the methionine metabolism 
 output <-    as.matrix( t(data_for_clustering ) ) [  69 : 86,   order_cellline ] 
...
# write.xlsx( data.frame( output)  , "figure2g_scaled_expression.xlsx", rowNames=TRUE)
