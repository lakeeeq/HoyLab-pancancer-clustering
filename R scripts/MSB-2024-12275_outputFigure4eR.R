...
input  <-data.frame( read.xlsx("Figure4e_right/figure4e_right_input.xlsx")) 
input[1:10, 1:10]
...
cluster_3_cellline <- c("NCIH226" ,  "HT144" , "BT20" , 
                        "A375"    ,  "Detroit562" , "BT474" , "OVCAR3",   "A549"     )
...
tT_no_same_drug <- tT
tT_no_same_drug$drug <- unlist(lapply(strsplit(rownames(tT), ".split.", fixed=T), `[` ,2))
tT_no_same_drug <- tT_no_same_drug[!duplicated(tT_no_same_drug$drug), ]

# there is only 19 non-overlapping drugs, therefore select these 19 drugs 
top_drug <- rownames( tT_no_same_drug[1:19, ] )
...
# write.xlsx( data.frame(input), "figure4e_right_scaled_AUC.xlsx", rowNames=TRUE)
